...
const arr = input
                ...
                .reduce((a, v) => {
                    v = v.map(e => e.split(':'))
                    // const entries = new Map(v)
                    const obj = Object.fromEntries(v)
                    if (obj.byr && obj.iyr && obj.eyr && obj.hgt && obj.hcl && obj.ecl && obj.pid)
                        a.push(obj)
                    return a
                }, [])
                

console.log(`The answer is: ${arr.length}`)


// PART TWO

const validatePassportFields = (obj) => {
    let { byr, iyr, eyr, hgt, hcl, ecl, pid } = obj

    // if (!byr || !iyr || !eyr || !hgt || !hcl || !ecl || !pid) return false

    const yearRangeTest = (year, minYear, maxYear) => {
        return year.length === 4 && Number(year) >= minYear && Number(year) <= maxYear ? true : false
    }

    const heightTest = (hgt) => {
        const test = /^[\d]+(cm|in)$/i.test(hgt)
        const measure = Number(hgt.match(/\d+/))
        const unit = String(hgt.match(/[a-z]+/i))

        if (test && (unit === 'cm' && measure >= 150 && measure <= 193) || (unit === 'in' && measure >= 59 && measure <= 76)) {
            return true
        } else {
            return false
        }
    ...
    const hairColorTest = (hcl) => {
        return /^#[0-9a-f]{6}$/i.test(hcl)
    }

    const eyeColorTest = (ecl) => {
        return /^(amb|blu|brn|gry|grn|hzl|oth)$/.test(ecl)
    }

    const passportIdTest = (pid) => {
        const digits = String(pid.match(/\d+/))

        return digits.length === 9 ? true : false
    }

    byr = yearRangeTest(byr, 1920, 2002)
    iyr = yearRangeTest(iyr, 2010, 2020)
    eyr = yearRangeTest(eyr, 2020, 2030)
    hgt = heightTest(hgt)
    hcl = hairColorTest(hcl)
    ecl = eyeColorTest(ecl)
    pid = passportIdTest(pid)
...
arr.forEach(e => validatePassportFields(e) && assertions++)
    
console.log(`The answer is: ${assertions}`)