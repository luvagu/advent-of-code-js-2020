...
console.time('validPasswordChecker time')

const validPasswordChecker = (arr) => {
    let assertions = 0

    arr.forEach(e => {
        const range = e[0].split('-').map(Number)
        const letter = e[1].replace(':', '')
        const min = range[0]
        const max = range[1]
        
        const len = e[2].split('').filter(e => e === letter).length

        if (len >= min && len <= max) assertions++
    })

    console.log(`The answer is: ${assertions}`)
}
 
validPasswordChecker(arr)

console.timeEnd('validPasswordChecker time')

// PART TWO

const input2 = `2-4 p: vpkpp
6-16 b: bbbbbbbbbbbbbbbpb
6-7 z: zzfzzdz
4-6 q: tfzqvqcpcmqqjqzd
7-8 k: rkkkknkw
...
console.time('validPasswordCheckerPos time')

const validPasswordCheckerPos = (arr) => {
    let assertions = 0

    arr.forEach(e => {
        const index = e[0].split('-').map(Number)
        const letter = e[1].replace(':', '')
        const firstPosLetter = e[2][index[0] - 1]
        const secondPosLetter = e[2][index[1] - 1]
        
        //console.log(letter, firstPosLetter, secondPosLetter)

        if (firstPosLetter !== secondPosLetter && (firstPosLetter === letter || secondPosLetter === letter)) assertions++
    })

    console.log(`The answer is: ${assertions}`)
}

validPasswordCheckerPos(arr)

console.timeEnd('validPasswordCheckerPos time')